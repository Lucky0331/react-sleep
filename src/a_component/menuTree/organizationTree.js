...
class OrTree extends React.Component {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.data !== this.props.data || nextProps.noShowId !== this.props.noShowId) {
        ...
        if (nextProps.modalShow !== this.props.modalShow && nextProps.modalShow) {
            ...
            this.setState({
                ...
                selected: (() => {
                    const temp = nextProps.data.find((item) => `${item.id}` === this.props.defaultKey[0]);
                    if (temp) {
    ...
    makeTreeDom(data) {
        return data.map((item, index) => {
            if (item.children) {
                return (
                    <TreeNode title={item.orgName} key={`${item.id}`} id={item.id} p={item.parentId} data={item}>
                        { this.makeTreeDom(item.children) }
                    </TreeNode>
                );
            } else {
                return <TreeNode title={item.orgName} key={`${item.id}`} id={item.id} p={item.parentId} data={item}/>;
            }
        });
    }
