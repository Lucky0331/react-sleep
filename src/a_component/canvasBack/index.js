...
class CanvasBack extends React.Component {
  ...
  init() {
    ...
    for (let i = 0; i < this.state.point; i++) {
      this.circleArr.push(
        this.drawCricle(
          c2d,
          this.num(canvas.width * 1.5, -canvas.width * 0.5),
          this.num(canvas.height * 1.5, -canvas.height * 0.5),
          this.num(10, 3),
          this.num(30, -30) / 40,
          this.num(30, -30) / 40
        )
  ...
  animate() {
    ...
    for (let i = 0; i < this.state.point; i++) {
      ...
      else if (cir.x < -this.myCanvas.width * 0.2)
        cir.x = this.myCanvas.width * 1.2;
      if (cir.y > this.myCanvas.height * 1.2)
        cir.y = -this.myCanvas.height * 0.2;
      else if (cir.y < -this.myCanvas.height * 0.2)
        cir.y = this.myCanvas.height * 1.2;
    }
