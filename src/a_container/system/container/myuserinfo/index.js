...
import { Form, Button, Icon, Input, Table, message, Popconfirm, Modal, Radio, InputNumber, Select, Tooltip, Divider, Cascader ,Popover } from 'antd';
    ...
    TmakeData(Tdata) {
        ...
        return Tdata.map((item2, index) => {
            return {
                ...
                eId: item2.id,
                citys: (item2.province && item2.city && item2.region) ? `${item2.province}/${item2.city}/${item2.region}` : '',
                serial: (index + 1) + ((this.state.pageNum - 1) * this.state.pageSize),
                age: item2.age,
                ...
                station: (item2.province && item2.city && item2.region) ? `${item2.province}/${item2.city}/${item2.region}` : '',
          ...
          >
              <Form>
                  ...
                  <FormItem
                    ...
                    {...formItemLayout}
                  >
                      {(!!this.state.nowData && this.state.nowData.headImg) ? this.state.nowData.headImg.split(',').map((item, index) => {
...
export default connect(
  ...
  (dispatch) => ({
    actions: bindActionCreators({ findAdminUserByKeys, addAdminUserInfo, deleteAdminUserInfo, updateAdminUserInfo, findAllRole, findAllRoleByUserId, assigningRole, findAllOrganizer, findAllProvince, findCityOrCounty, findStationByArea,findUserInfo ,myCustomers}, dispatch),
  })
