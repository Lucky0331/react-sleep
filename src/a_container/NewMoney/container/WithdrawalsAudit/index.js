...
import { Form, Button, Icon, Input, Table, message, Modal, Tooltip, Popconfirm, Select, Divider ,DatePicker ,Checkbox,Popover} from 'antd';
import './index.scss';
...
import {onChange, onOk, onChange4, warning, findProductTypeByWhere, WithdrawalsAudit, WithdrawalsAuditEgis} from '../../../../a_action/shop-action';
...
class Category extends React.Component {
    ...
    onGetData(pageNum, pageSize) {
        const params = {
            ...
            minApplyTime: this.state.searchBeginTime ? `${tools.dateToStr(this.state.searchBeginTime.utc()._d)}` : '',
            maxApplyTime: this.state.searchEndTime ? `${tools.dateToStr(this.state.searchEndTime.utc()._d)} `: '',
        };
   ...
   onAddNewShow() {
    ...
    onAdopt(){
        ...
        this.props.actions.WithdrawalsAuditEgis(params).then((res) => {
            if (res.status === 200) {
    ...
    makeColumns(){
        const columns = [
            ...
            },
            {
                ...
                render: (text, record) => {
                    const controls = [];
                    (record.withdrawStatus === 3 ) && controls.push(
                        <Popconfirm  title="确定审核通过吗?" onConfirm={() =>this.onAdoptAlone(record)} okText="确定" cancelText="取消">
                        ...
                        <Popover
                            content={
                                <div>
                                    ...
                                    {/*</ul>*/}
                                    <div style={{marginLeft:'125px'}}><a onClick={(e) => this.onAdoptNoAlone(record)}>确定</a></div>
                                </div>
                            }
                            title="请输入拒绝理由"
                            trigger="click"
            ...
            <div className="page-refundaudit">
                <div className="system-search">
                  <ul className="search-ul more-ul">
                      ...
                      <li>
                          <Button icon="download" style={{color: '#fff',backgroundColor:'#108ee9',borderColor: '#108ee9'}} onClick={warning2}>导出</Button>
                      </li>
                   </ul>
              </div>
              <div className="system-table">
                  <div style={{marginTop:'10px',marginBottom:'10px'}}>
                      ...
                      <span style={{margin:'0 10px 0 0'}}>
                          ...
                          />反选</span>
                      <Popconfirm  title="确定批量审核通过吗?" placement="bottom" onConfirm={() => this.onAdopt()} okText="确定" cancelText="取消">
                          <Button type="primary"
                          ...
                          content={
                              <div>
                                  ...
                                  <ul style={{display: 'flex',justifyContent: 'space-between',}}>
                                      ...
                                      <li style={{width:'40px'}}><a onClick={(e) => this.hide(e)}>取消</a></li>
                                  </ul>
                              </div>
                          }
                          title="请输入拒绝理由"
              ...
              >
                <Form>
                    ...
                    <FormItem
                        ...
                        {...formItemLayout}
                    >
                        {!!this.state.nowData ? this.WithdrawStatus(this.state.nowData.withdrawStatus) : ''}
                    </FormItem>
                    <FormItem
                        label="提现方式"
                        {...formItemLayout}
                    >
                        {!!this.state.nowData ? this.WithdrawType(this.state.nowData.withdrawType) : ''}
                    </FormItem>
                    <FormItem
                        label="用户id"
                        {...formItemLayout}
...
export default connect(
    ...
    (dispatch) => ({
        actions: bindActionCreators({ onChange,onOk ,onChange4,warning,findProductTypeByWhere,WithdrawalsAudit,WithdrawalsAuditEgis}, dispatch),
    })
